...
export interface BorderFloatingFieldProps {
  label: string;
  value: string | number;
  onChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
  type?: React.HTMLInputTypeAttribute;
...
export const BorderFloatingField = React.forwardRef<
  HTMLInputElement,
  BorderFloatingFieldProps
>(
  (
    {
      label,
...
BorderFloatingField.displayName = "BorderFloatingField";
      ...
      >
        ...
        <Label
          className={cn(
            ...
            (hasValue || value) && "-top-3 text-xs text-slate-500",
            disabled && "opacity-70",
          )}
        >
          {label} {required && <span className="text-rose-500">*</span>}
...
export function SectionDetailsHeader({
  title,
  description,
  icon: Icon,
}: {
